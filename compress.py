...
def load_json_safe(path):
    """Load a JSON, return None if invalid or empty."""
    try:
...
def replace_empty_flame_params(flame_init_flame_params_path):
    # Find first non-empty JSON
    valid_json = None
    for file in sorted(os.listdir(flame_init_flame_params_path)):
        if file.endswith('.json'):
            full_path = os.path.join(flame_init_flame_params_path, file)
            data = load_json_safe(full_path)
            if data is not None:
                valid_json = data
                print(f"Found first valid flame_params file: {file}")
                break

    if valid_json is None:
        print("❌ No valid JSON found in the folder!")
        return
    ...
    for file in sorted(os.listdir(flame_init_flame_params_path)):
        if file.endswith('.json'):
            ...
            data = load_json_safe(full_path)
            if data is None:
                # Overwrite with the valid one
                with open(full_path, 'w') as f:
                    json.dump(valid_json, f)
                print(f"🔄 Replaced empty file: {file}")
...
#replace_empty_flame_params(flame_init_flame_params_path)
    



# Example usage
# replace_empty_flame_params('/path/to/flame_init/flame_params')
...
# data = np.load('/scratch/aparna/PHOENIX-2014-T_compressed/train/01April_2010_Thursday_heute-6696.npz', allow_pickle=True)
# print(data["smplx_optimized"].item(0).keys())
# print(data["keypoints"].item(0).keys())
