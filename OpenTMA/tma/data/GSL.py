...
from .humanml.data.dataset import (
    Text2MotionDatasetGSL,
    Text2MotionDatasetMotionX_text_all,
)
def load_smplx_model(model_path, gender='neutral', device='cpu',batch_size=1):
    return smplx.create(model_path=model_path, model_type='smplx',create_global_orient=True,use_pca=False,use_joints_conf=True,use_hands=True, use_face=True, num_betas=10,batch_size=batch_size,
        num_expression_coeffs=50,flat_hand_mean= False,
    use_face_contour=True).to(device=device, dtype=torch.float32)

class GSL_DataModule(BASEDataModule):
    ...
    def __init__(
        ...
        if cfg.DATASET.JOINT_TYPE == "humanml3d":
            self.njoints = 22
        elif cfg.DATASET.JOINT_TYPE == "motionx" or "asl" or "gsl":
            self.njoints = 52
        else:
        ...
        if phase == "text_only":
            ...
            if cfg.model.condition in [
                ...
                self.Dataset = Text2MotionDatasetGSL
