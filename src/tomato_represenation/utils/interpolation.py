...
def savitzky_golay(y, window_size, order, deriv=0, rate=1,pose_rep="matrix"):
    from easyconvert import to_matrix, matrix_to
    x = []
    for i in range(len(y)):
        y[i] = torch.tensor(y[i])
        mid = to_matrix(pose_rep, y[i])
        x.append(matrix_to("quaternion", mid).numpy())

    x = np.array(x)
    print(x.shape)
    smoothed = savgol_filter(x, window_size, order,axis=0)
    for i in range(len(smoothed)):
        
        mid = to_matrix("quaternion", torch.from_numpy(smoothed[i]))
        mid = matrix_to(pose_rep, mid).detach().cpu()
